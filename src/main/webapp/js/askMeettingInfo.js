function addpageInfo_askMeettingInfo(e,page_Info){
		$(page_Info).empty();
		$(page_Info).append("当前第 "+e.myMessage.pageNum+" 页,总共 "
				+e.myMessage.pages+" 页,总共 "
				+e.myMessage.total+" 记录");
	}
	function addPageNav_askMeettingInfo(e,page_nav){
		//添加翻页导航栏
		$(page_nav).empty();
		...
		if(e.myMessage.hasPreviousPage == false){
			...
			prePageLi.click(function() {
				askMeettingInfo(e.myMessage.pageNum - 1);
			});
		...
		if(e.myMessage.hasNextPage == false){
			...
			lastPageLi.click(function(){
				askMeettingInfo(e.myMessage.pages);
			});
		}
